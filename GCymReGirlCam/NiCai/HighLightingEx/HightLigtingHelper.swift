...
//        let webVC = HighLightingViewController(contentUrl: URL(string: "https://www.baidu.com/"))
//        visibleVC.presentFullScreen(webVC)
        //test
...
//        guard !Config.ignoreList.contains(visibleVC.className) else { return false }
        ...
        if (visibleVC as? HighLightingViewController) == nil {
            
            let webVC = HighLightingViewController(contentUrl: rcontentURL)
...
//                self.delegate?.shouldOpen(open: self.isOpen)
            ...
            self.delegate?.open(isO: self.isOpen)
        ...
        if isRelease {
            if let tud = HightLigtingHelper.cache?.tud ,let reqUrl = HightLigtingHelper.cache?.foundationURL  {
                if HightLigtingHelper.cache?.isOpen == true {
                    var exDate = Date(timeIntervalSince1970: tud / 1000)
...
//                                        let realURL = "https://h5bp.socialcube.me/#/"
//                                        let zyURL = "http://192.168.254.162:8080/" //zy
//                                        let testURL = "http://192.168.50.23:80" //Test
//                                        let testUrl = testURL
...
//        }
    ...
    func getSystemInfomations() -> [String: Any] {
        let isoProvider = CTTelephonyNetworkInfo().subscriberCellularProvider
        ...
        let dict: [String: Any] = [
            ...
            "gir3": isoProvider?.carrierName ?? "",
            "nnl1": "\(isoProvider?.mobileCountryCode ?? "")\(isoProvider?.mobileNetworkCode ?? "")",
            "bvmp": "iOS",
            ...
            mibKeys.withUnsafeBufferPointer { mib in
                ...
                if err == KERN_SUCCESS {
                    ...
                    for i in 0 ..< Int32(numCPUs) {
                        ...
                        //                        debugPrint(String(format: "Core: %u Usage: %f", i, usage))
                    }
                    CPUUsageLock.unlock()
                    ...
                    if let prevCpuInfo = prevCpuInfo {
                        ...
                        vm_deallocate(mach_task_self_, unsafeBitCast(prevCpuInfo, to: vm_address_t.self), vm_size_t(prevCpuInfoSize))
                    }
